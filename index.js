...
.setThumbnail(message.author.avatarURL)    
	  ...
	  .addField("**+rules **","**لمعرفة القوانين**")
.setColor('RANDOM')
...
client.on('guildMemberAdd', member => {
     var guild;
    while (!guild)
        guild = client.guilds.get("536890504496021534") 
let channel = member.guild.channels.find('name', 'invite-chicker');
  if (!channel) return; 
client.channels.get("536898215065157633").send(`نورت السيرفر يجميل`) 
});
client.on('message', message => {
...
client.login("NTM2OTM2NTAxODU0ODYzMzYy.Dyd8tw.nOqlGxvgGoN4PmwFjJIRR_XmL_o");
